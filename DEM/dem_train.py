...
parser.add_argument(
    '--data',
    default = os.path.join('data', 'data.hdf5'),
    type    = str,
    help    = "Data to be loaded for training. Default is 'data/data.hdf5'."
    )
...
keys = list(f.keys())
print(keys)
X = np.empty(f['bub_X'].shape)
f['bub_X'].read_direct(X)
Y = np.empty(f['bub_Y'].shape)
f['bub_Y'].read_direct(Y)   
f.close()

print(X[1,:])
print(Y[1,:])
input_names = ["x1", "x2", "x3", "z1", "z2", "z3", "z4", "z5", "z6", "z7", "sin"]
